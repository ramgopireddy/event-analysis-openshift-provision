...
OPENSHIFT_DM7_TEMPLATES_TAG=7.4.0.GA
...
function import_imagestreams_and_templates() {
  ...
  oc create -f https://raw.githubusercontent.com/jboss-container-images/rhdm-7-openshift-image/$OPENSHIFT_DM7_TEMPLATES_TAG/rhdm74-image-streams.yaml

  echo_header "Patching the ImageStreams"
  oc patch is/rhdm74-decisioncentral-openshift --type='json' -p '[{"op": "replace", "path": "/spec/tags/0/from/name", "value": "registry.access.redhat.com/rhdm-7/rhdm74-decisioncentral-openshift:1.0"}]'
  oc patch is/rhdm74-kieserver-openshift --type='json' -p '[{"op": "replace", "path": "/spec/tags/0/from/name", "value": "registry.access.redhat.com/rhdm-7/rhdm74-kieserver-openshift:1.0"}]'

  echo_header "Importing Templates"
  oc create -f https://raw.githubusercontent.com/jboss-container-images/rhdm-7-openshift-image/$OPENSHIFT_DM7_TEMPLATES_TAG/templates/rhdm74-authoring.yaml
}
...
function create_application() {
  ...
  oc new-app --template=rhdm74-authoring \
